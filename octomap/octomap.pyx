...
ctypedef defs.OccupancyOcTreeBase[defs.ColorOcTreeNode].tree_iterator* c_tree_iterator_ptr
ctypedef defs.OccupancyOcTreeBase[defs.ColorOcTreeNode].leaf_iterator* c_leaf_iterator_ptr
ctypedef defs.OccupancyOcTreeBase[defs.ColorOcTreeNode].leaf_bbx_iterator* c_leaf_bbx_iterator_ptr
...
cdef class ColorOcTreeNode:
    ...
    def setColor(self, unsigned int r, unsigned int g, unsigned int b):
        if self.thisptr:
            self.thisptr.setColor(r, g, b)
        else:
            raise NullPointerException
...
cdef class c_tree_iterator(c_iterator_base):
    """
    Iterator over the complete tree (inner nodes and leafs).
    """
    def __cinit__(self):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[c_tree_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
    ...
    def isLeaf(self):
        if self.__is_acceseable():
            return defs.static_cast[c_tree_iterator_ptr](self.thisptr).isLeaf()
        else:
            raise NullPointerException
...
cdef class c_leaf_iterator(c_iterator_base):
    """
    Iterator over the complete tree (leafs).
    """
    def __cinit__(self):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[c_leaf_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
...
cdef class c_leaf_bbx_iterator(c_iterator_base):
    """
    Iterator over the complete tree (leafs).
    """
    def __cinit__(self):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[c_leaf_bbx_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
...
cdef class ColorOcTree:
    ...
    def __cinit__(self, arg):
        ...
        #else:
        #    self.thisptr = new defs.ColorOcTree(string(<char*?>arg))
    ...
    def readBinary(self, filename):
        cdef defs.istringstream iss
        if filename.startswith(b"# Octomap ColorOcTree binary file"):
            iss.str(string(<char*?>filename, len(filename)))
            return self.thisptr.readBinary(<defs.istream&?>iss)
        else:
            return self.thisptr.readBinary(string(<char*?>filename))
    ...
    #            raise NullPointerException
    #    else:
    #        return self.thisptr.isNodeOccupied(<defs.ColorOcTreeNode>deref(deref((<tree_iterator>node).thisptr)))
    ...
    #            raise NullPointerException
    #    else:
    #        return self.thisptr.isNodeAtThreshold(<defs.ColorOcTreeNode>deref(deref((<tree_iterator>node).thisptr)))
